        ...
        >
          <header className="flex items-center justify-between border-b border-slate-200 px-6 py-3">
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-slate-500">{entries.length} real or explicitly simulated tool steps</p>
            </div>
            <button
            ...
            {[...entries].reverse().map((entry, reverseIndex) => (
              <li className="flex items-start gap-3 border-b border-slate-100 py-3 last:border-0" key={entry.id}>
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[11px] text-slate-500">{entry.tool} · {entry.description}</p>
                </div>
              </li>
            ))}
          </ol>
